...
often misunderstood that `del x` is not the equivalent of `x.__del__()`.  The former (`del x`) simple decrements
the reference count of `x` by 1.  The latter `x.__del__()` is invoked when no references remain.
